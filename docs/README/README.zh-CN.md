...
**Re:Music**是基于Flutter的原生桌面端音频文件管理工具。它支持读取 metadata 对音频文件进行批量重命名、编辑音频元数据（音乐标签）、在线获取元数据（开发中），以及音频降采样与格式转换。
...
*   **批量重命名**: 支持读取音频元数据，提供灵活的重命名规则配置，支持 `{artist}` 与 `{albumArtist}` 占位符
*   **音乐标签编辑**: 支持区分编辑曲目艺术家与专辑艺术家，支持批量修改与保存
*   **在线获取元数据（开发中）**: 从在线数据源获取歌曲标签与封面图片
*   **音频降采样**: 支持降采样与格式转换，提供无损规格目标与 MP3 输出选项。
*   **文件管理**: 支持拖拽导入文件夹或文件，文件列表筛选与排序
*   **个性化**: 内置亮色与暗色模式，支持 Hue 滑块调色（MD3），支持中英文多语言
...
## 命名模板 (Naming Patterns)

应用使用基于占位符的命名模板：

*   `{artist}`：曲目艺术家（回退顺序：参与创作艺术家 -> 专辑艺术家 -> 未知艺术家）
*   `{albumArtist}`：专辑艺术家
*   `{title}`：曲目标题
*   `{album}`：专辑名
*   `{track}`：音轨号
...
示例：`{albumArtist} - {track} - {title}` -> `Album Artist - 01 - Track Title.mp3`
...
*   **Core Dependencies**:
    *   `audio_metadata_reader`: 跨格式音频元数据快速解析。
    *   `audiotags`: 细粒度音频标签读取与写入（例如曲目艺术家、专辑艺术家）。
